...
func (s *EventService) CreateEventsFromUserMessage(telegramID int64, messages []model.TextMessage) ([]model.ScheduledEvent, error) {
	...
	events, err := s.aiService.ExtractCalendarEvents(user.TimeZone, &messages)
	if err != nil {
		return nil, err
	}
	...
	for i, event := range *events {
		scheduledEvent, err := s.calendarServices[model.ProviderGoogle].CreateEvent(user.ID, user.TimeZone, &event)
		if err != nil {
			return nil, err
		}
